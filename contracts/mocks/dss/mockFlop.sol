...
contract MockFlopper is MockLibNote {
    // --- Auth ---
    ...
    // --- Auction ---
    // auth modifier removed
    function kick(address gal, uint lot, uint bid) external returns (uint id) {
    ...
    function mockSetEnded(uint id) external {
        bids[id].end = uint48(0);
    }
}